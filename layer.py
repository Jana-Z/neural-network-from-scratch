...
class DenseLayer(Layer):
    def __init__(self, n, activation_function='sigmoid', weights=np.empty(0), biases=np.empty(0), prev_layer=None, next_layer=None, depth=None):
        '''input params:
            ...
            activation_function: activation_function given as a string'''
        if type(n) == int:
            self.n = n
        else:
            print('number of neurons has to be a whole number')
            sys.exit()
        # initialized in initialize_params
        self.weights  = weights
        self.biases = biases
        self.depth = depth
        # Initialized in __init__ of ScratchNet.py
    ...
    def initialize_params(self):
        ...
        if np.array_equal(self.biases, np.empty(0)):
            self.biases = np.random.randn(self.n)
    ...
    def compute_cost_gradients(self, targets, cost_function):
        '''computes the gradients for the output layer
            special case since the gradients in the ouput layer only rely on the gradients of the cost function
